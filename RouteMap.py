...
def routes_of_distance(G, source, distance, route='', cumulative=False, use_weights=False):
    ...
    for n in G.successors(source):
        ...
        if new_distance >= 0:
            new_routes = routes_of_distance(G, n, new_distance, route=route, cumulative=cumulative, use_weights=use_weights)
...
class RouteMap:
    ...
    def find_number_routes(self, source, target, distance, distance_type):
        ...
        if (distance_type == RouteMap.DistanceType.MAX_STOPS):
            all_routes = routes_of_distance(self.G, source, distance, cumulative=True, use_weights=False)
        elif (distance_type == RouteMap.DistanceType.EXACT_STOPS):
            all_routes = routes_of_distance(self.G, source, distance, cumulative=False, use_weights=False)
        elif (distance_type == RouteMap.DistanceType.MAX_DISTANCE):
            all_routes = routes_of_distance(self.G, source, distance, cumulative=True, use_weights=True)
        else:
            raise ValueError
    ...
    def find_shortest_route(self, source, target):
        ...
        for n in self.G.successors(source):
            try:
                curr_distance = self.G[source][n]['weight'] + nx.shortest_path_length(self.G, source=n, target=target, weight='weight')
            except:
                continue
